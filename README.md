...
| **Result** | **55.6% of manual review removed**, at a threshold chosen out-of-fold and never on the split it is reported against. The escape rate there is 0.66% of defect-labelled candidates (95% interval 0.43%–1.02%) against QP-110's 0.5%: **on that reading the budget is not met**. Per defect, which is how QP-110 is written, the re-verifier escapes 0.35% and the whole line 0.51%. That is one seed: over five re-runs of the whole procedure the median is 50.9% at 0.50%, range 49.0%–55.6% ([seed variance](#what-a-re-run-lands-on)). |
| **Stack** | Python 3.12 · PyTorch (MPS / CPU) · LangGraph · MCP · FastAPI + Jinja · SQLite · Ollama (`gpt-oss:20b`) |
| **Verification** | 1,442 tests, no model or GPU required. Every threshold cites its source; every figure names the script that produced it. |
...
- **Two rates, both printed.** 0.66% counts *candidates* carrying a defect
  label that were dismissed (20 of 3,018). Counting *defects* — a defect
  escapes only if every candidate covering it is dismissed, which is how
  QP-110 is written — the re-verifier escapes 0.35% (11 of 3,135 that reached
  it) and the whole line 0.51%, including the 0.16% no threshold recovers.
  Neither reading is the flattering one to quote alone.
- **The selection set does not predict the deployment set.** The procedure
  that chose 0.912 estimated 0.32% out-of-fold; this split measures 0.66%,
...
- **Prevalence.** The previous headline, 56.2% on 8,143 candidates, was measured
  ...
  review reduction fell to 52.8% at the then-shipped threshold while regions
  reaching an operator fell from 3,568 to 3,457. Review reduction is quoted with the prevalence it assumes.
- **Routing.** 85.9% of candidates are dispositioned by the re-verifier alone,
  at 2.5 ms each on CPU (p50); the LLM is called on the remaining 14.1%.
...
gradient. With the threshold set where recall peaks, the re-verifier — at the
0.961 it shipped with on 2026-08-26 — dismisses **1,387 of 2,953 real defects — a 47% escape rate** on a queue that
...
against 55.6% for differencing plus the re-verifier on DeepPCB. Two lines and
...
**1,442 tests.** 1,417 of them run on a clean checkout in CI — they build their
...
- **The whole-line escape rate is 0.51%, and it is two numbers rather than
  one.** 0.16% of defects (5 of 3,140 on the test split) have no candidate on
  them at all — nothing recovers those. A further 0.35% are flagged and then
  dismissed by the re-verifier, which is what the dismissal threshold governs.
  It read 0.61% (0.22% + 0.38%) until 2026-08-31: that run predates the
  registration stage, which flags two of the defects it had counted as
  unreachable.
...
- **The escape budget is exceeded on the aggregate too, since 2026-08-31.**
  At the shipped threshold `short` escapes at 1.77% and `open` at 1.16%
  against an aggregate of 0.66%, and which class is worst moves with the
  checkpoint — it was `open` at 1.35% before the 2026-08-24 retrain. The
  aggregate met the budget until the threshold stopped being swept on the
  split it is reported on. They are confident errors, so no cut on the model's own
...
- **Two of this project's twenty invariants are only partly guarded, and one
  cannot be guarded at all.** `CLAUDE.md` states twenty rules that must not be
