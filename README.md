...
every flagged region goes to a person, and most of what they look at is nothing.
This project puts a ResNet-18 re-verifier in front of that queue — **52.8% of
manual review removed at a ≤0.5% escape budget** — and a LangGraph agent behind
it for the cases the model cannot settle, ending in a durable hand-off to an
operator. A second entrance, `/ask`, answers a supervisor's plain-language
question with a validated plan of lookups and a chart drawn from the data.
Everything runs locally; every number below names the script that produced it.

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/diagrams/disposition-flow-dark.svg">
  <img alt="How one flagged region is dispositioned: the re-verifier classifies it; a confident false call is dismissed and a confident defect confirmed without a language model; anything else gathers context, the LLM writes a rationale, and the region is decided on the classifier's confidence or handed to an operator through a durable interrupt." src="docs/diagrams/disposition-flow-light.svg" width="100%">
</picture>

<sub>One flagged region, start to finish. Rendered from the graph's own node names and
the thresholds the code carries — `scripts/render_diagrams.py`. The `/ask` flow is
drawn [further down](#asking-the-line-a-question--ask).</sub>
...
- **What it does.** A ResNet-18 re-verifier reads every AOI candidate and settles
  82% of them in 2.5 ms each; the rest go through a LangGraph flow that gathers
  production context and acceptance criteria, has a local LLM *explain* the
  region, and hands what it cannot settle to an operator through a durable
  interrupt. A second entrance, `/ask`, turns a supervisor's plain-language
  question into a validated, typed plan of lookups and draws the chart from the
  result shape. Both are bilingual (繁體中文 / English).
- **Headline.** At a ≤0.5% escape budget, **52.8% of manual review is removed**
  on 7,322 candidates from 499 unseen boards, 41.2% of them genuine defects —
  and the 95% interval on that escape rate reaches 0.82%, which does not exclude
  exceeding the budget. Reported that way on purpose.
- **What is measured rather than claimed.** Every threshold cites a script or a
  document line and 29 tests fail if one drifts. Six things this project
  measured turned out to be wrong — three in its own favour — and each has a
  section below with the number before and after.
- **Where it stops working, measured.** On photographed boards (HRIPCB) the
  differencing front end fails at the first gate; on solder-paste images
  (PCB-AoI) a YOLO26n detector finds 92% of defects but its confidence removes
  only 1.2% of review at the same budget. Both are findings, not gaps — see
  [where it stops working](#where-it-stops-working-two-more-datasets-two-refusals).
- **1,274 tests**, none of which call the model or need a GPU; 25 want the
  dataset on disk and are marked.

## Demo

<!-- VIDEO PLACEHOLDER — Mike: replace the paragraph below with the link once
     recorded (docs/demo.mp4 via git-lfs, or a YouTube link). Shot list and
     narration: docs/demo-script.md -->

**Demo video: coming.** Two minutes — run a board → the queue → open a region
→ press `0` for "I can't tell" → a senior answers what an operator could not
→ `/boards` → ask `/ask` whether M32's parameter change moved its open share
→ the control machine where it did not → switch the page to English.

Until then, the station as it runs today (every page also reads in 繁體中文):

| The queue — what the agent could not settle, oldest first | One region — template, board, difference, and why it was handed over |
|---|---|
| ![queue](docs/screenshots/queue-en.png) | ![region](docs/screenshots/region-en.png) |
| **`/boards` — held, released and waiting, counted over the table** | **`/ask` — a validated plan, the fan-out, a chart from the result shape, prose beside it** |
...
`uv run pytest` runs the 1,274 tests with no model, no GPU and no dataset.
The full list of measurement scripts, the container, and what each command
writes is under [Running it](#running-it).

## Result

On the official DeepPCB test split — 7,322 AOI candidates from 499 unseen
boards, of which 3,018 (41.2%) are genuine defects:
...
**These numbers went down when the system got better, and that is worth one
paragraph.** Until 2026-08-26 this table read 56.2% at the ≤0.50% budget over
8,143 candidates. That run predates the registration stage: with it turned on
the detector stopped emitting the easy false calls that residual misalignment
was manufacturing, so the candidate population fell to 7,322 — 842 fewer false
calls and 21 *more* real defects found. Review reduction is `dismissed /
total`, so a cleaner denominator lowers it. What actually reaches an operator
fell: **3,568 regions before, 3,457 after.** The component metric got worse and
the line got better, which is why this project reports prevalence beside every
reduction figure — see `docs/benchmarks.md`, "Prevalence".
...
Two more things the table does not say and the reader needs. The 0.50% is 15
escapes in 3,018 defects *on this split*; read as the rate on unseen defects,
the 95% interval runs to **0.82%**, so a deployment cannot claim the budget is
met from this evidence alone. And the budget is one number over six classes
that the work instructions do not treat alike: at the shipped threshold
**`short` escapes at 1.55%**, three times the aggregate, and it is a class no
document admits an acceptable instance of. Which class exceeds moves with the
checkpoint (`open` did, before the 2026-08-24 retrain). The obvious fix, a
class-aware veto, does not exist and cannot be built from the model's own
output: on the opens it dismisses, `P(open)` is below 0.006 — confident errors,
not uncertain ones. What closes them is a second measurement that does not
share the failure, which is electrical test.
...
The interesting part of this repository is not the pipeline. It is the six
...
One line each here; the full account of every one, with the numbers before
and after, is in [docs/findings.md](docs/findings.md).
...
## Where it stops working: two more datasets, two refusals
...
**1,274 tests.** 1,249 of them run on a clean checkout in CI — they build their
