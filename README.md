...
| **Result** | **55.6% of manual review removed** at a 0.66% escape rate (95% interval 0.43%–1.02%), against QP-110's 0.5% — **on that reading the budget is not met**. The threshold was chosen out-of-fold, never on the split it is reported against, and this is one seed of five ([Results](#results)). |
...
`uv run pytest` runs the suite without a model, a GPU or the dataset. Every
measurement script, the container and the full CLI: [Running it](#running-it).
...
[LICENSE](LICENSE) · the road as a deck, with the questions an interviewer
asks: [docs/deck/](docs/deck/)
