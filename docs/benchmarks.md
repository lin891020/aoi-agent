...
SQL arm after the two sentences the 2026-08-29 adjudication named: a SELECT filters only on entities the question names, and before/after an event is the event tools. Control arm follows, same prompt.
...
| severity | questions | correct | should answer | should refuse |
|---|---|---|---|---|
| core | 51 | 32/51 = 63% | 18/28 = 64% | 14/23 = 61% |
| boundary | 18 | 11/18 = 61% | 10/13 = 77% | 1/5 = 20% |
| stretch | 1 | 0/1 = 0% | 0/1 = 0% | — |
...
**7 of the 70 cannot be passed by any plan at all, and are counted as misses above.** The grader pinned `defect_type` on a `search_standards`-only plan; `search_standards` takes `query` and has no such parameter, so `validate_plan` would throw out any plan that tried to satisfy the expectation. That is a grading error, recorded rather than repaired — the fixture marks them `fixture_defect` and a guard test asserts the list is exactly these. Excluding them, the score over the remaining 63 is 42/63 = 67%. Both numbers are here on purpose: the first is what the set as graded says, the second is what it says about the planner.
...
**Plans `validate_plan` threw out.** 2 of 70 did not validate, 1 of which had scored a hit on tools and arguments and so would be counted correct above while running nothing. The usual cause is a `days` beyond the 9 the store holds. Scoring counts the plan, so these are reported here rather than folded into the table.
...
- S06 **boundary** 今天早班到現在總共 flag 幾個區域？ — should have refused; planned run_sql()
  planned: `run_sql()`
  graded: 'Flagged regions' is the candidate count including false calls, which no aggregate tool returns, and the A-shift restriction is not expressible either.
- S10 **boundary** 那台新的上線以後 short 有沒有變多？ — should have refused; planned query_machine_events(machine_id='M32') + query_defect_history(defect_type='short', machine_id='M32') + query_machine_stats(defect_type='short')
  planned: `query_machine_events(machine_id='M32') + query_defect_history(defect_type='short', machine_id='M32') + query_machine_stats(defect_type='short')`
  graded: '那台新的' names no machine and the question text does not even name the line, so any machine_id would be a guess presented as a finding.
- S11 **core** 現在還有幾片卡在那邊等人看？ — should have refused; planned run_sql()
  planned: `run_sql()`
  graded: The escalation queue is not among the five plannable tools, so this surface cannot count what is waiting on a person.
...
- S25 **boundary** C 班交接說 M32 有動過參數，動完之後出來的結果有沒有差？ — should have refused; planned query_machine_events(machine_id='M32') + query_defect_history(machine_id='M32') + query_defect_history(machine_id='M32')
  ...
  graded: A before/after comparison needs the time of the change, which the question does not give and no tool records, and no tool returns a per-day series to split on anyway.
- S28 **boundary** 這片有三個 open，還救得回來嗎還是直接報廢？ — never queried defect_type=['open']
  planned: `search_standards() + query_board_context(board='<board_id>')`
  graded: WI-201 gives the disposition rule — scrap or route to jumper repair per repair class, with Class 3 barred from jumper repair without customer concession — which is the decision being made at the machine.
  defect_type pinned on search_standards -- no tool in the expected plan takes that argument, so no plan can satisfy this expectation and the question scores a miss whatever the planner does
- S31 **core** Which station has the highest false call rate this week? — should have refused; planned query_false_call_rate()
  planned: `query_false_call_rate()`
...
- A04 **core** B 班的 short 是不是比 A 班多?晚班的人手比較生。 — should have refused; planned run_sql()
  planned: `run_sql()`
  graded: Shift is stored on the board row but no aggregate tool accepts it as a filter, so A and B cannot be counted separately.
- A10 **core** L2-M22 的 open 那麼多,是因為蝕刻液老化了嗎? — matched no accepted plan: the grader's primary plan — refused a question it should have answered; machine_id='M22' with defect_type='open' gives the count directly, though without the fleet comparison that makes it meaningful — refused a question it should have answered
  planned: `(refused)`
  graded: The counts confirm the premise and are worth fetching; the etch-chemistry cause cannot be established because no tool holds a single process parameter.
- A11 **core** 稽核要我附 open 的判定依據,work instruction 裡面是怎麼寫的? — never queried defect_type=['open']
  planned: `search_standards()`
...
- A20 **core** 品保問 mousebite 的允收標準,我們的工單有寫嗎? — never queried defect_type=['mousebite']
  ...
  graded: WI-203 states the mousebite acceptance limits — 80% remaining conductor width, not extending more than two conductor widths — and one retrieval reaches it.
  defect_type pinned on search_standards -- no tool in the expected plan takes that argument, so no plan can satisfy this expectation and the question scores a miss whatever the planner does
- A21 **core** L1 跟 L3 這 5 天,哪一條線每片的缺陷數比較差? — never called ['query_defect_history']
  planned: `run_sql()`
  graded: defects_per_board is returned per call and only per call, so comparing two lines needs one call for each with the same window.
- A22 **core** 上次 review 20085294 的是誰?他判得對不對? — should have refused; planned run_sql()
  planned: `run_sql()`
...
- A25 **core** mousebite 跟 spur 是不是集中在同一台機台? — never called ['query_machine_stats']
  planned: `run_sql()`
  graded: Only query_machine_stats resolves a class to machines, and it takes one class per call, so co-location needs two rankings compared.
- A26 **core** 上次出問題的那台機台,現在有沒有好一點? — should have refused; planned run_sql() + query_defect_history(date_from='2026-08-03', date_to='2026-08-09', machine_id='<machine_id_from_previous_call>') + query_defect_history(date_from='2026-07-27', date_to='2026-08-02', machine_id='<machine_id_from_previous_call>')
  planned: `run_sql() + query_defect_history(date_from='2026-08-03', date_to='2026-08-09', machine_id='<machine_id_from_previous_call>') + query_defect_history(date_from='2026-07-27', date_to='2026-08-02', machine_id='<machine_id_from_previous_call>')`
  graded: 'The machine that had the problem last time' refers to shared context the tools do not hold, so any machine_id would be a guess dressed as a lookup.
- A32 **core** 工單對 open 的允收條件是怎麼寫的? — never queried defect_type=['open']
  planned: `search_standards()`
  graded: A single retrieval reaches WI-201, and production counts have no bearing on what the document says.
  defect_type pinned on search_standards -- no tool in the expected plan takes that argument, so no plan can satisfy this expectation and the question scores a miss whatever the planner does
...
- 這兩個禮拜 pin-hole 出現幾次? — call 1: days=14 exceeds the 9 days of data held, which would silently return the whole span
- 上次出問題的那台機台,現在有沒有好一點? — call 2: machine_id='<machine_id_from_previous_call>' does not exist (known values: M11, M12, M21, M22, M31, M32); call 3: machine_id='<machine_id_from_previous_call>' does not exist (known values: M11, M12, M21, M22, M31, M32); call 3: date_from='2026-07-27' is outside the days held (2026-08-01 to 2026-08-09); there is nothing on that day to count
...
- S01 昨天大夜 L2 那邊是不是有出什麼事？我早上進來看板子堆在那邊。
  `query_defect_history(date_from='2026-08-08', date_to='2026-08-08', line_id='L2') + query_machine_events(machine_id='M21') + query_machine_events(machine_id='M22')` — matched: the grader's primary plan
- S02 M21 跟 M22 這禮拜的 false call 差多少？
  `query_false_call_rate()`
- S03 20085294 這片客戶說有問題，當初我們是怎麼判的？
  `query_board_context(board='20085294') + list_candidates(board='20085294') + run_sql()` — matched: the grader's primary plan
- S04 看一下 L2
  `query_defect_history(line_id='L2') + query_defect_history(line_id='L1') + query_defect_history(line_id='L3')`
- S05 open 到什麼程度算 reject？WI 裡面怎麼寫的？
  `search_standards()`
...
- S08 去年同一個禮拜這條線的 defect rate 多少？
  `(refused)`
- S09 M22 怎樣
  `query_machine_stats(defect_type=None) + query_defect_history(machine_id='M22')` — matched: the grader's primary plan
- S10 那台新的上線以後 short 有沒有變多？
  `query_machine_events(machine_id='M32') + query_defect_history(defect_type='short', machine_id='M32') + query_machine_stats(defect_type='short')`
- S11 現在還有幾片卡在那邊等人看？
  `run_sql()`
- S12 M31 昨天的 FC rate?
  `run_sql()`
- S13 有一片 2008 開頭的，好像是 5294 還是 5249，客訴那批的，幫我找一下。
  `run_sql()`
- S14 L3 這兩天 mousebite 一直冒出來，你覺得是什麼原因？
  `query_defect_history(date_from='2026-08-08', date_to='2026-08-09', defect_type='mousebite', line_id='L3') + query_defect_history(date_from='2026-08-08', date_to='2026-08-09', defect_type='mousebite', line_id='L1') + query_defect_history(date_from='2026-08-08', date_to='2026-08-09', defect_type='mousebite', line_id='L2') + search_standards()` — matched: the grader's primary plan
- S15 這個 lot 裡面 open 跟 short 各幾個？
  `(refused)`
- S16 早班跟大夜的 false call 差很多嗎？同一台機器比。
  `(refused)`
- S17 copper 這個月變多，是真的變多還是我們判得比較嚴？
  `(refused)`
- S18 連續三片同一個位置有 open，我要不要停線？
  `query_defect_history(defect_type='open') + query_machine_stats(defect_type='open') + search_standards()`
- S19 L2 這個月的 defect rate 跟上個月比怎樣？
  `(refused)`
- S20 這片是誰判的？我要問他當初看到什麼。
  `(refused)`
...
- S27 這個 lot 是哪一台跑的？
  `(refused)`
- S28 這片有三個 open，還救得回來嗎還是直接報廢？
  `search_standards() + query_board_context(board='<board_id>')`
- S29 照現在這個速度，這班結束前大概還會有幾片要人看？
  `(refused)`
- S30 上一季 L1 的 escape 有幾件？
  `(refused)`
...
- S34 我們判 false call 放行的，後面有沒有被客戶抓到過？
  `(refused)`
- S35 我昨天休假，這兩天有沒有哪一台狀況要我特別注意的？
  `query_defect_history(date_from='2026-08-08', date_to='2026-08-09') + query_machine_stats(date_from='2026-08-08', date_to='2026-08-09')` — matched: the grader's primary plan
- A01 不要只看單一種缺陷,整體算下來這幾天哪一台機台最該排保養?
  `query_machine_stats(defect_type=None, top_n=None)`
- A02 上個月 L1 的 open 趨勢怎麼樣?有沒有愈來愈糟?
  `(refused)` — matched: reading '上個月' strictly as the calendar month before the data means zero overlap, which makes a refusal that states the 9-day limit defensible
...
- A22 上次 review 20085294 的是誰?他判得對不對?
  `run_sql()`
- A23 L3 的 M31 這 3 天 copper 幾件?
  `query_defect_history(defect_type='copper', line_id='L3', machine_id='M31')`
- A24 這兩個禮拜 pin-hole 出現幾次?
  `query_defect_history(defect_type='pin-hole')` — matched: the grader's primary plan
- A25 mousebite 跟 spur 是不是集中在同一台機台?
  `run_sql()`
- A26 上次出問題的那台機台,現在有沒有好一點?
  `run_sql() + query_defect_history(date_from='2026-08-03', date_to='2026-08-09', machine_id='<machine_id_from_previous_call>') + query_defect_history(date_from='2026-07-27', date_to='2026-08-02', machine_id='<machine_id_from_previous_call>')`
- A27 AOI 在 20085294 上框的那幾個點,照工單標準哪些算 critical?
  `list_candidates(board='20085294') + search_standards()` — matched: the grader's primary plan
- A28 我要跟廠長報告,把 L3 這 7 天的缺陷數字給我。
  `query_defect_history(line_id='L3')`
...
Control arm (AOI_SQL_TOOL=0) under the same prompt as the entry above.
...
| severity | questions | correct | should answer | should refuse |
|---|---|---|---|---|
| core | 51 | 37/51 = 73% | 18/28 = 64% | 19/23 = 83% |
| boundary | 18 | 9/18 = 50% | 7/13 = 54% | 2/5 = 40% |
| stretch | 1 | 0/1 = 0% | 0/1 = 0% | — |
...
**Planner failures.** 4 question(s) produced no plan at all (model unreachable, or a response that would not parse). These score as misses, not as refusals: a timeout that counted as a refusal would make a contended machine look well-calibrated.
...
- S25 **boundary** C 班交接說 M32 有動過參數，動完之後出來的結果有沒有差？ — no plan was produced: the planner did not answer (ReadTimeout)
  ...
  graded: A before/after comparison needs the time of the change, which the question does not give and no tool records, and no tool returns a per-day series to split on anyway.
- S28 **boundary** 這片有三個 open，還救得回來嗎還是直接報廢？ — never queried defect_type=['open']
  planned: `search_standards() + query_board_context(board='<板號>')`
  graded: WI-201 gives the disposition rule — scrap or route to jumper repair per repair class, with Class 3 barred from jumper repair without customer concession — which is the decision being made at the machine.
  defect_type pinned on search_standards -- no tool in the expected plan takes that argument, so no plan can satisfy this expectation and the question scores a miss whatever the planner does
- S31 **core** Which station has the highest false call rate this week? — should have refused; planned query_false_call_rate()
  planned: `query_false_call_rate()`
...
- S33 **core** short 要不要 100% 重驗？WI 有沒有寫？ — never queried defect_type=['short']
  ...
  defect_type pinned on search_standards -- no tool in the expected plan takes that argument, so no plan can satisfy this expectation and the question scores a miss whatever the planner does
- A01 **core** 不要只看單一種缺陷,整體算下來這幾天哪一台機台最該排保養? — matched no accepted plan: the grader's primary plan — never queried defect_type=['open', 'short', 'mousebite', 'spur', 'copper', 'pin-hole']; one call per machine_id gives the all-class total per machine directly, which is closer to '整體算下來' than six per-class rankings — never called ['query_defect_history']
  planned: `query_machine_stats(defect_type=None)`
  graded: 'Overall, across all classes' requires a fan-out because query_machine_stats takes exactly one class, and the question explicitly rules out the single-class shortcut.
- A10 **core** L2-M22 的 open 那麼多,是因為蝕刻液老化了嗎? — matched no accepted plan: the grader's primary plan — refused a question it should have answered; machine_id='M22' with defect_type='open' gives the count directly, though without the fleet comparison that makes it meaningful — refused a question it should have answered
  planned: `(refused)`
  graded: The counts confirm the premise and are worth fetching; the etch-chemistry cause cannot be established because no tool holds a single process parameter.
- A11 **core** 稽核要我附 open 的判定依據,work instruction 裡面是怎麼寫的? — never queried defect_type=['open']
  ...
  graded: WI-201 is in the indexed documents and retrieval is the only capability needed; the audit framing changes nothing.
  defect_type pinned on search_standards -- no tool in the expected plan takes that argument, so no plan can satisfy this expectation and the question scores a miss whatever the planner does
- A12 **core** 20085294 這片跟同一個 lot 的其他板子比,缺陷算多還是少? — no plan was produced: the planner did not answer (ReadTimeout)
  planned: `(no plan)`
  graded: query_board_context supplies the lot average (lot_boards, lot_defects, lot_defects_per_board) but not this board's own count, which only list_candidates provides — both halves are needed to say 'more or less'.
- A15 **core** scratch 這種刮傷最近是不是變多了? — should have refused; planned query_defect_history(date_from='2026-08-07', date_to='2026-08-09', defect_type='mousebite') + query_defect_history(date_from='2026-08-01', date_to='2026-08-06', defect_type='mousebite')
  planned: `query_defect_history(date_from='2026-08-07', date_to='2026-08-09', defect_type='mousebite') + query_defect_history(date_from='2026-08-01', date_to='2026-08-06', defect_type='mousebite')`
  graded: The classifier and the store know six classes and scratch is not one, so there is nothing to count under that name.
- A18 **boundary** M11 表現怎麼樣? — matched no accepted plan: the grader's primary plan — never called ['query_defect_history']; a per-class fan-out places M11 against the fleet, which is what 'how is it performing' implies — never queried defect_type=['open', 'short', 'mousebite', 'spur', 'copper', 'pin-hole']
  planned: `query_machine_stats(defect_type=None, top_n=None)`
  graded: A valid machine with no metric named is the same shape as S04 and S09, and query_defect_history(machine_id='M11') returns the all-class breakdown without needing a class to be chosen.
- A20 **core** 品保問 mousebite 的允收標準,我們的工單有寫嗎? — never queried defect_type=['mousebite']
  ...
  graded: WI-203 states the mousebite acceptance limits — 80% remaining conductor width, not extending more than two conductor widths — and one retrieval reaches it.
  defect_type pinned on search_standards -- no tool in the expected plan takes that argument, so no plan can satisfy this expectation and the question scores a miss whatever the planner does
- A21 **core** L1 跟 L3 這 5 天,哪一條線每片的缺陷數比較差? — no plan was produced: the planner did not answer (ReadTimeout)
  planned: `(no plan)`
  graded: defects_per_board is returned per call and only per call, so comparing two lines needs one call for each with the same window.
- A23 **core** L3 的 M31 這 3 天 copper 幾件? — never queried line_id=['L3']
  planned: `query_defect_history(defect_type='copper', machine_id='M31')`
...
- A32 **core** 工單對 open 的允收條件是怎麼寫的? — never queried defect_type=['open']
  ...
  graded: A single retrieval reaches WI-201, and production counts have no bearing on what the document says.
  defect_type pinned on search_standards -- no tool in the expected plan takes that argument, so no plan can satisfy this expectation and the question scores a miss whatever the planner does
- A35 **boundary** open 跟 short 哪一個問題比較大? — matched no accepted plan: the grader's primary plan — never called ['query_defect_history']; 'bigger problem' has a severity reading as well as a volume one, and the documents are the only source for severity — never called ['query_defect_history', 'search_standards']; one filtered call per class, equivalent to reading two entries out of a single unfiltered by_class — never called ['query_defect_history']
  planned: `query_machine_stats(defect_type='open') + query_machine_stats(defect_type='short')`
  graded: A single unfiltered query_defect_history returns by_class with both counts, so the volume reading is one call away, and the metric chosen simply has to be named.
...
- S08 去年同一個禮拜這條線的 defect rate 多少？
  `(refused)`
- S09 M22 怎樣
  `query_machine_stats(defect_type='open') + query_machine_stats(defect_type='short') + query_machine_stats(defect_type='mousebite') + query_machine_stats(defect_type='spur') + query_machine_stats(defect_type='copper') + query_machine_stats(defect_type='pin-hole') + query_false_call_rate()` — matched: per-class fan-out ranks M22 against the fleet, which is the more useful reading of 'how is it doing'
- S10 那台新的上線以後 short 有沒有變多？
  `(refused)`
- S11 現在還有幾片卡在那邊等人看？
  `(refused)`
...
- S13 有一片 2008 開頭的，好像是 5294 還是 5249，客訴那批的，幫我找一下。
  `(refused)`
- S14 L3 這兩天 mousebite 一直冒出來，你覺得是什麼原因？
  `query_defect_history(date_from='2026-08-08', date_to='2026-08-09', defect_type='mousebite', line_id='L3') + query_machine_stats(defect_type='mousebite')` — matched: the grader's primary plan
- S15 這個 lot 裡面 open 跟 short 各幾個？
  `(refused)`
- S16 早班跟大夜的 false call 差很多嗎？同一台機器比。
  `(refused)`
...
- S27 這個 lot 是哪一台跑的？
  `(refused)`
- S28 這片有三個 open，還救得回來嗎還是直接報廢？
  `search_standards() + query_board_context(board='<板號>')`
- S29 照現在這個速度，這班結束前大概還會有幾片要人看？
  `(refused)`
- S30 上一季 L1 的 escape 有幾件？
  `(refused)`
...
- S34 我們判 false call 放行的，後面有沒有被客戶抓到過？
  `(refused)`
- S35 我昨天休假，這兩天有沒有哪一台狀況要我特別注意的？
  `query_machine_stats(date_from='2026-08-08', date_to='2026-08-09', top_n=5)` — matched: the grader's primary plan
- A01 不要只看單一種缺陷,整體算下來這幾天哪一台機台最該排保養?
  `query_machine_stats(defect_type=None)`
- A02 上個月 L1 的 open 趨勢怎麼樣?有沒有愈來愈糟?
  `(refused)` — matched: reading '上個月' strictly as the calendar month before the data means zero overlap, which makes a refusal that states the 9-day limit defensible
- A03 20085294 這片客訴板,幫我把生產履歷調出來,哪條線哪台機台哪個班次做的。
  `query_board_context(board='20085294')`
...
- A14 L2 這 7 天 open 幾件?順便幫我估一下這些重工的成本大概多少錢。
  `query_defect_history(defect_type='open', line_id='L2')`
- A15 scratch 這種刮傷最近是不是變多了?
  `query_defect_history(date_from='2026-08-07', date_to='2026-08-09', defect_type='mousebite') + query_defect_history(date_from='2026-08-01', date_to='2026-08-06', defect_type='mousebite')`
- A16 這個月的良率有沒有改善?
  `(refused)`
- A17 spur 跟 mousebite 長得很像,但我要問的是 spur,這幾天各機台的狀況?
  `query_machine_stats(defect_type='spur')`
...
- A34 LOT-2608003 這個 lot 這幾天總共幾件缺陷?
  `query_defect_history()`
- A35 open 跟 short 哪一個問題比較大?
  `query_machine_stats(defect_type='open') + query_machine_stats(defect_type='short')`

</details>

### Adjudication — the SQL tool re-measured under the two sentences, SQL arm against control

2026-08-29, afternoon. The two entries above this one are the re-run the
morning's adjudication called for: the prompt now says a SELECT may filter
only on entities the question names, and that before/after an event is the
event tools. Same seventy, same model, both arms, whole plans kept
(`--raw`); the per-question table with every SELECT and its verdict is the
report published beside this entry
(https://claude.ai/code/artifact/8aab45e3-f898-41c1-a774-4e2345faac16; the
generator and the adjudication JSON are in the session scratchpad, and every
figure in it is derivable from the two `--raw` files the eval wrote).

| | SQL arm | control | morning, SQL arm | morning, control |
|---|---|---|---|---|
| should answer | **28/42** | 25/42 (four timeouts: S06, S25, A12, A21) | 24/42 | 26/42 |
| should refuse, raw | 15/28 | 21/28 | 15/28 | 21/28 |
| should refuse, adjudicated | **23/28** | 25/28 | 22/28 | 25/28 |
| determinism | 60/70 | 63/70 | 60/70 | 60/70 |
| planner timeouts | 0 | 4 | 0 | 3 |

**The two sentences did what they were written to do.** S25 (*M32 動過參數*)
is composed on the event tools again in the SQL arm -- every repeat -- and
S20 (*這片是誰判的*, no board named) is refused again. Neither cost the five
questions the tool bought: S06, S12, A04 (a shift, a machine on a day) are
still SELECTs, S11 is still a SELECT, S02 moved to `query_false_call_rate`.
The answer side came back to 28/42, which is the best figure this set has
produced, with S03 (how *we* judged 20085294 -- `review_decisions`, which no
typed tool reads) and S01 (L2's night shift, the shift filter the typed tool
lacks) now answered through SQL beside the typed tool.

**Fourteen SELECTs, adjudicated one by one** (the report carries the SQL):
six are a route that did not exist (S03, S06, S12, S13, S23, A04); one is
either route (S01); three should have been the typed tool (A12, A21, A25 --
`query_board_context`, `query_defect_history` twice, `query_machine_stats`
twice, each of which carries the baseline the SELECT drops); one names no
entity (A26, on one repeat of three); one asks for ground truth (A22, second
half); and **two are the failure the no-SQL rule was written for**: S11
counted `board_dispositions.disposition = 'pending'`, a state that does not
exist (the states are `held` and `released`; what waits on a person is
`escalations.status`), and S24 grouped by `index_on_board` as though it were a
position. Both returned a number. Both numbers were wrong in a way nothing on
the page could show.

**What that finding changed, after the measurement.** The guard now refuses a
bare `column = literal` (or `IN`) that matches no row and names the values
that exist -- `validate_plan`'s `line_id="L4"` rule, applied to a WHERE
clause. It would have refused S11's SELECT and A12/A22's `boards.id =
'20085294'` (the board number is `stem`). It cannot see S24: `index_on_board`
holds values, they just do not mean what the query took them to mean. **That
guard is not in either arm above**; it is held by `tests/test_sql_guard.py`
and is the next thing a re-run measures.

**The control's four timeouts.** Both control arms today timed out (three in
the morning, four now) and neither SQL arm did. The station made no model call
during either run and `ollama ps` was clean at both ends; the control arm ran
second both times, into the afternoon. It is not explained, it is named, and
it makes the control's answer-side figure (25/42) a floor: three of the four
were answerable rows.

**Reading, and the rule it supports.** With the two sentences, the tool
answers six questions no typed tool could, costs one adjudicated refusal
against the control (23/28 to 25/28: S11's wrong SELECT and A26's guess, both
now addressed -- one by the guard, one by a rule already in the prompt that
held on two repeats of three), and lifts the answer side. The rule the
evidence supports, in the order a planner should apply it: a typed tool that
...
SQL arm, third run: the guard now refuses an equality on a value no row holds (plan-only, so the guard is exercised only by validate_plan's rejections; the SELECT text is kept in --raw and checked against the guard offline).
...
| should answer | 42 | 28/42 = 67% |
| should refuse | 28 | 15/28 = 54% |
| determinism | 70 | 58/70 = 83% planned the same tools across 3 runs |
...
**Planner failures.** 2 question(s) produced no plan at all (model unreachable, or a response that would not parse). These score as misses, not as refusals: a timeout that counted as a refusal would make a contended machine look well-calibrated.
...
- A22 **core** 上次 review 20085294 的是誰?他判得對不對? — should have refused; planned run_sql()
  planned: `run_sql()`
  graded: No tool returns a reviewer identity, and judging whether a call was right would require ground_truth, which is held for evaluation only and must never reach an operator.
- A25 **core** mousebite 跟 spur 是不是集中在同一台機台? — no plan was produced: the planner did not answer (ReadTimeout)
  planned: `(no plan)`
  graded: Only query_machine_stats resolves a class to machines, and it takes one class per call, so co-location needs two rankings compared.
- A26 **core** 上次出問題的那台機台,現在有沒有好一點? — no plan was produced: the planner did not answer (ReadTimeout)
  planned: `(no plan)`
  graded: 'The machine that had the problem last time' refers to shared context the tools do not hold, so any machine_id would be a guess dressed as a lookup.
- A32 **core** 工單對 open 的允收條件是怎麼寫的? — never queried defect_type=['open']
  planned: `search_standards()`
...
### Adjudication — the SQL tool's third run, and the literal guard read against every SELECT it wrote

2026-08-29, evening. The planner entry above this one is the SQL arm run a
third time, with no change to the prompt and the guard's new rule in place:
28/42 answered, 15/28 refused raw, 58/70 stable, two timeouts (A25, A26).
With the prompt unchanged the planner wrote the same SELECTs as the second
run, nearly to the character -- the eval is close to deterministic, which is
what makes the second run's per-question reading hold rather than being one
draw. Scores are on plans and the guard acts after the plan, so its effect is
read offline: every SELECT the planner wrote in this run, given to
`sql_guard.check` against a snapshot of the store.

| SELECT | guard | why |
|---|---|---|
| S11, both variants (`disposition = 'pending'`) | **refused** | no such value; the states held are `held` |
| A22, both variants (`boards.id = '20085294'`) | **refused** | `id` is the integer key; 500 distinct values, none of them the board number |
| S13, first variant (`disposition = 'complaint'`) | **refused** | no such value; the second variant, a plain `LIKE` on `stem`, passes |
| S01, second variant (events for M22) | **refused** | M22 has no recorded event; an empty table would have read as "nothing happened" |
| S24, second variant | refused, parse error | sqlglot could not parse it -- a parser limit, costing an answer rather than a number |
| the other 17 | pass | |

The two SELECTs the second run adjudicated as *valid and wrong* split the
way the guard's docstring predicted: S11's wrong value is refused with the
values that exist; S24's `index_on_board`-as-position is not, because the
values exist and only the meaning is wrong -- that class is left to the prompt
...
`gpt-oss:20b`, 25 hand-written questions, each asked 3 times. Plans are scored, not answers: the tools are deterministic, so a correct plan yields correct data by construction and the errors live in the plan. The store held 9 days at the time of the run.

listed-set rule and tenth few-shot, 2026-08-30
...
| | questions | correct |
|---|---|---|
| should answer | 16 | 14/16 = 88% |
| should refuse | 9 | 9/9 = 100% |
| determinism | 25 | 24/25 = 96% planned the same tools across 3 runs |

**Held out from the prompt.** 5 of the 25 questions are few-shot examples verbatim or near-paraphrases, so on those the model is reciting rather than planning. On the remaining 20 it scored 19/20 = 95%, with 19/20 = 95% stable. Read that one rather than the headline above, and read it narrowly: it is agreement with one author's expected plans on question shapes that author chose. It does not bound the questions nobody thought to ask, the `days` and `top_k` arguments that go unscored, or whether the prose written over a correct plan is correct.

**Plans `validate_plan` threw out.** 0 of 25 did not validate, 0 of which had scored a hit on tools and arguments and so would be counted correct above while running nothing. The usual cause is a `days` beyond the 9 the store holds. Scoring counts the plan, so these are reported here rather than folded into the table.
...
- 2026-07-30 缺陷數量前 5 名的機台
  `(refused)`
- 最差的那台機台，最近有沒有發生什麼事？
  `query_machine_stats(defect_type=None, top_n=1) + query_machine_events(machine_id=None)`
- 哪條線的 open 最多？那條線的缺陷組成是什麼？
  `query_machine_stats(defect_type='open') + query_defect_history(line_id='L1') + query_defect_history(line_id='L2') + query_defect_history(line_id='L3')`
- 上次出問題的那台機台現在怎麼樣？
  `(refused)`
...
|---|---|---|---|---|
| core | 51 | 32/51 = 63% | 18/28 = 64% | 14/23 = 61% |
| boundary | 18 | 10/18 = 56% | 8/13 = 62% | 2/5 = 40% |
| stretch | 1 | 0/1 = 0% | 0/1 = 0% | — |
...
**Planner failures.** 1 question(s) produced no plan at all (model unreachable, or a response that would not parse). These score as misses, not as refusals: a timeout that counted as a refusal would make a contended machine look well-calibrated.
...
- S13 **stretch** 有一片 2008 開頭的，好像是 5294 還是 5249，客訴那批的，幫我找一下。 — matched no accepted plan: the grader's primary plan — never called ['query_board_context']; once the right stem is identified, listing its flagged regions is what '找一下' is for — never called ['list_candidates', 'query_board_context']
  planned: `run_sql()`
  graded: Both spellings are well-formed board ids and query_board_context returns a clean error for one that does not exist, so probing both resolves the question without guessing.
- S16 **core** 早班跟大夜的 false call 差很多嗎？同一台機器比。 — should have refused; planned run_sql()
  planned: `run_sql()`
  graded: Two independent blockers: shift is recorded on the board row but no aggregate tool filters by it, and false calls are excluded from every count.
- S17 **boundary** copper 這個月變多，是真的變多還是我們判得比較嚴？ — matched no accepted plan: the grader's primary plan — refused a question it should have answered; share_of_defects for copper separates 'more copper' from 'more of everything', which is the closest available proxy for the question's real distinction — refused a question it should have answered
  planned: `(refused)`
...
- A11 **core** 稽核要我附 open 的判定依據,work instruction 裡面是怎麼寫的? — never queried defect_type=['open']
  ...
  defect_type pinned on search_standards -- no tool in the expected plan takes that argument, so no plan can satisfy this expectation and the question scores a miss whatever the planner does
- A12 **core** 20085294 這片跟同一個 lot 的其他板子比,缺陷算多還是少? — matched no accepted plan: the grader's primary plan — never called ['list_candidates']; query_defect_history(lot_id=...) re-derives the lot average within a window, a useful cross-check on board_context's unwindowed figure — never called ['list_candidates', 'query_defect_history']
  planned: `query_board_context(board='20085294') + run_sql() + run_sql()`
  graded: query_board_context supplies the lot average (lot_boards, lot_defects, lot_defects_per_board) but not this board's own count, which only list_candidates provides — both halves are needed to say 'more or less'.
- A20 **core** 品保問 mousebite 的允收標準,我們的工單有寫嗎? — never queried defect_type=['mousebite']
  planned: `search_standards()`
  graded: WI-203 states the mousebite acceptance limits — 80% remaining conductor width, not extending more than two conductor widths — and one retrieval reaches it.
...
- S06 今天早班到現在總共 flag 幾個區域？
  `run_sql()`
- S07 這兩天 M12 出來的東西我看怪怪的，講不上來，你幫我看一下有沒有什麼不對。
  `query_defect_history(date_from='2026-08-08', date_to='2026-08-09', machine_id='M12')` — matched: the grader's primary plan
- S08 去年同一個禮拜這條線的 defect rate 多少？
  `(refused)`
- S09 M22 怎樣
  `query_machine_stats(defect_type=None) + query_defect_history(machine_id='M22')` — matched: the grader's primary plan
...
- S13 有一片 2008 開頭的，好像是 5294 還是 5249，客訴那批的，幫我找一下。
  `run_sql()`
- S14 L3 這兩天 mousebite 一直冒出來，你覺得是什麼原因？
  `query_defect_history(defect_type='mousebite', line_id='L3') + query_defect_history(line_id='L3')` — matched: the grader's primary plan
- S15 這個 lot 裡面 open 跟 short 各幾個？
  `(refused)`
- S16 早班跟大夜的 false call 差很多嗎？同一台機器比。
  `run_sql()`
...
- S34 我們判 false call 放行的，後面有沒有被客戶抓到過？
  `(refused)`
- S35 我昨天休假，這兩天有沒有哪一台狀況要我特別注意的？
  `query_machine_stats(date_from='2026-08-07', date_to='2026-08-08', top_n=None) + query_false_call_rate()` — matched: the grader's primary plan
- A01 不要只看單一種缺陷,整體算下來這幾天哪一台機台最該排保養?
  `query_machine_stats(defect_type=None)`
- A02 上個月 L1 的 open 趨勢怎麼樣?有沒有愈來愈糟?
  `(refused)` — matched: reading '上個月' strictly as the calendar month before the data means zero overlap, which makes a refusal that states the 9-day limit defensible
...
- A11 稽核要我附 open 的判定依據,work instruction 裡面是怎麼寫的?
  `search_standards()`
- A12 20085294 這片跟同一個 lot 的其他板子比,缺陷算多還是少?
  `query_board_context(board='20085294') + run_sql() + run_sql()`
- A13 最近 short 有比較多嗎?
  `query_defect_history(defect_type='short') + query_defect_history()` — matched: the grader's primary plan
- A14 L2 這 7 天 open 幾件?順便幫我估一下這些重工的成本大概多少錢。
  `query_defect_history(defect_type='open', line_id='L2')`
...
- A17 spur 跟 mousebite 長得很像,但我要問的是 spur,這幾天各機台的狀況?
  `query_machine_stats(defect_type='spur')`
- A18 M11 表現怎麼樣?
  `query_machine_stats(defect_type=None) + query_defect_history(machine_id='M11')` — matched: the grader's primary plan
- A19 把這份 L2 的數據匯出成 Excel,寄給我主管。
  `(refused)`
- A20 品保問 mousebite 的允收標準,我們的工單有寫嗎?
  `search_standards()`
...
- A34 LOT-2608003 這個 lot 這幾天總共幾件缺陷?
  `query_defect_history()`
- A35 open 跟 short 哪一個問題比較大?
  `query_defect_history(defect_type='open') + query_defect_history(defect_type='short') + query_machine_stats(defect_type='open') + query_machine_stats(defect_type='short')` — matched: the grader's primary plan
...
listed-set rule with its two limits, 2026-08-30

Machine at start: **busy** -- 59906 44% /opt/homebrew/Cellar/python@3.11/3.11.15_1/Frameworks/Python.framework/Versions/3.11/Resources/Python.app/Cont.

Registry: `run_sql` **in** (the SQL arm; `AOI_SQL_TOOL=0` for the control).

| | questions | correct |
|---|---|---|
| should answer | 16 | 15/16 = 94% |
| should refuse | 9 | 8/9 = 89% |
| determinism | 25 | 25/25 = 100% planned the same tools across 3 runs |

**Held out from the prompt.** 5 of the 25 questions are few-shot examples verbatim or near-paraphrases, so on those the model is reciting rather than planning. On the remaining 20 it scored 18/20 = 90%, with 20/20 = 100% stable. Read that one rather than the headline above, and read it narrowly: it is agreement with one author's expected plans on question shapes that author chose. It does not bound the questions nobody thought to ask, the `days` and `top_k` arguments that go unscored, or whether the prose written over a correct plan is correct.

**Plans `validate_plan` threw out.** 3 of 25 did not validate, 2 of which had scored a hit on tools and arguments and so would be counted correct above while running nothing. The usual cause is a `days` beyond the 9 the store holds. Scoring counts the plan, so these are reported here rather than folded into the table.
...
- 哪一台機器的缺陷率最高？ — matched no accepted plan: one query_defect_history per machine: defects_per_board is the overall rate directly — never called ['query_defect_history']; one query_machine_stats per defect class: the six classes are exactly the non-false_call set, so summing each machine's per_board across them is the same overall rate — never queried defect_type=['open', 'short', 'mousebite', 'spur', 'copper', 'pin-hole']
  planned: `query_machine_stats(top_n=1)`
- 上次出問題的那台機台現在怎麼樣？ — should have refused; planned run_sql() + query_defect_history(machine_id='<machine_id_from_previous_call>') + query_machine_events(machine_id='<machine_id_from_previous_call>')
  planned: `run_sql() + query_defect_history(machine_id='<machine_id_from_previous_call>') + query_machine_events(machine_id='<machine_id_from_previous_call>')`
  graded: 2026-08-30: 'had trouble last time' is not a measurable thing in the store, so no ranking picks the machine out; still a refusal

Rejected plans:

- M31 跟 M32 誰比較穩？ — call 1: query_machine_stats has no argument 'machine_id' (it takes defect_type, days, date_from, date_to, top_n); call 2: query_machine_stats has no argument 'machine_id' (it takes defect_type, days, date_from, date_to, top_n)
- 最差的那台機台，最近有沒有發生什麼事？ — call 2: machine_id='<machine_id_from_previous_call>' is a placeholder, not a value -- the question does not say which machine_id is meant
- 上次出問題的那台機台現在怎麼樣？ — call 2: machine_id='<machine_id_from_previous_call>' is a placeholder, not a value -- the question does not say which machine_id is meant; call 3: machine_id='<machine_id_from_previous_call>' is a placeholder, not a value -- the question does not say which machine_id is meant
...
- 2026-07-30 缺陷數量前 5 名的機台
  `(refused)`
- 最差的那台機台，最近有沒有發生什麼事？
  `query_machine_stats(defect_type=None, top_n=1) + query_machine_events(machine_id='<machine_id_from_previous_call>')`
- 哪條線的 open 最多？那條線的缺陷組成是什麼？
  `query_machine_stats(defect_type='open') + query_defect_history(line_id='L1') + query_defect_history(line_id='L2') + query_defect_history(line_id='L3')`
- 上次出問題的那台機台現在怎麼樣？
  `run_sql() + query_defect_history(machine_id='<machine_id_from_previous_call>') + query_machine_events(machine_id='<machine_id_from_previous_call>')`
...
|---|---|---|
| should answer | 42 | 28/42 = 67% |
| should refuse | 28 | 18/28 = 64% |
| determinism | 70 | 59/70 = 84% planned the same tools across 3 runs |

Broken down by how much a failure would matter. The severities are the grader's, set before any run:

| severity | questions | correct | should answer | should refuse |
|---|---|---|---|---|
| core | 51 | 34/51 = 67% | 19/28 = 68% | 15/23 = 65% |
| boundary | 18 | 12/18 = 67% | 9/13 = 69% | 3/5 = 60% |
| stretch | 1 | 0/1 = 0% | 0/1 = 0% | — |
...
- S17 **boundary** copper 這個月變多，是真的變多還是我們判得比較嚴？ — matched no accepted plan: the grader's primary plan — refused a question it should have answered; share_of_defects for copper separates 'more copper' from 'more of everything', which is the closest available proxy for the question's real distinction — refused a question it should have answered
  ...
  graded: The copper count over the held window is fetchable and informative; the criterion-drift half is not, because no tool exposes thresholds or confidence over time.
- S18 **boundary** 連續三片同一個位置有 open，我要不要停線？ — never called ['search_standards']
  planned: `run_sql() + query_machine_stats(defect_type='open')`
  graded: The documents carry the relevant rules — WI-201 makes any confirmed open critical, QP-110 escalates when a lot has produced two or more confirmed criticals, WI-300 escalates on repeat coordinates in the same lot — even though no document authorises a line stop.
  defect_type pinned on search_standards -- no tool in the expected plan takes that argument, so no plan can satisfy this expectation and the question scores a miss whatever the planner does
- S20 **core** 這片是誰判的？我要問他當初看到什麼。 — should have refused; planned run_sql()
  planned: `run_sql()`
...
- S31 **core** Which station has the highest false call rate this week? — should have refused; planned query_false_call_rate()
  planned: `query_false_call_rate()`
  graded: query_machine_stats ranks machines by a real defect class only; false calls are excluded from it and from query_defect_history, so no false-call ranking exists.
- S32 **boundary** 我覺得最近人在看的量變多了，是不是機器越判越沒把握？ — should have refused; planned query_defect_history() + query_false_call_rate()
  planned: `query_defect_history() + query_false_call_rate()`
  graded: Both halves need history this surface does not have: escalation volume over time and the model's confidence distribution over time, neither exposed by any tool.
- S33 **core** short 要不要 100% 重驗？WI 有沒有寫？ — never queried defect_type=['short']
  planned: `search_standards()`
  graded: WI-202 speaks to this directly — no short is acceptable at any product class, and rework is followed by optical and electrical re-inspection — so the document either settles the argument or is shown not to state a 100% rule.
  defect_type pinned on search_standards -- no tool in the expected plan takes that argument, so no plan can satisfy this expectation and the question scores a miss whatever the planner does
- A01 **core** 不要只看單一種缺陷,整體算下來這幾天哪一台機台最該排保養? — matched no accepted plan: the grader's primary plan — never queried defect_type=['open', 'short', 'mousebite', 'spur', 'copper', 'pin-hole']; one call per machine_id gives the all-class total per machine directly, which is closer to '整體算下來' than six per-class rankings — never called ['query_defect_history']
  planned: `query_machine_stats(defect_type=None, top_n=1)`
  graded: 'Overall, across all classes' requires a fan-out because query_machine_stats takes exactly one class, and the question explicitly rules out the single-class shortcut.
- A04 **core** B 班的 short 是不是比 A 班多?晚班的人手比較生。 — should have refused; planned run_sql()
  planned: `run_sql()`
  graded: Shift is stored on the board row but no aggregate tool accepts it as a filter, so A and B cannot be counted separately.
...
- S13 有一片 2008 開頭的，好像是 5294 還是 5249，客訴那批的，幫我找一下。
  `run_sql()`
- S14 L3 這兩天 mousebite 一直冒出來，你覺得是什麼原因？
  `query_defect_history(line_id='L3') + query_machine_stats(defect_type='mousebite')` — matched: the grader's primary plan
- S15 這個 lot 裡面 open 跟 short 各幾個？
  `(refused)`
- S16 早班跟大夜的 false call 差很多嗎？同一台機器比。
  `(refused)`
- S17 copper 這個月變多，是真的變多還是我們判得比較嚴？
  `(refused)`
- S18 連續三片同一個位置有 open，我要不要停線？
  `run_sql() + query_machine_stats(defect_type='open')`
- S19 L2 這個月的 defect rate 跟上個月比怎樣？
  `(refused)`
- S20 這片是誰判的？我要問他當初看到什麼。
  `run_sql()`
...
- S31 Which station has the highest false call rate this week?
  `query_false_call_rate()`
- S32 我覺得最近人在看的量變多了，是不是機器越判越沒把握？
  `query_defect_history() + query_false_call_rate()`
- S33 short 要不要 100% 重驗？WI 有沒有寫？
  `search_standards()`
- S34 我們判 false call 放行的，後面有沒有被客戶抓到過？
  `(refused)`
- S35 我昨天休假，這兩天有沒有哪一台狀況要我特別注意的？
  `query_machine_stats()` — matched: the grader's primary plan
- A01 不要只看單一種缺陷,整體算下來這幾天哪一台機台最該排保養?
  `query_machine_stats(defect_type=None, top_n=1)`
- A02 上個月 L1 的 open 趨勢怎麼樣?有沒有愈來愈糟?
  `(refused)` — matched: reading '上個月' strictly as the calendar month before the data means zero overlap, which makes a refusal that states the 9-day limit defensible
- A03 20085294 這片客訴板,幫我把生產履歷調出來,哪條線哪台機台哪個班次做的。
  `query_board_context(board='20085294')`
...
- A16 這個月的良率有沒有改善?
  `(refused)`
- A17 spur 跟 mousebite 長得很像,但我要問的是 spur,這幾天各機台的狀況?
  `query_machine_stats(defect_type='spur') + query_defect_history(defect_type='spur')`
- A18 M11 表現怎麼樣?
  `query_defect_history(machine_id='M11')` — matched: the grader's primary plan
- A19 把這份 L2 的數據匯出成 Excel,寄給我主管。
  `(refused)`
...
- A30 把 scratch 加進缺陷類別,以後這一類要單獨統計。
  `(refused)`
- A31 20085294 上面 AOI 抓到哪些點?那片是誰做的?哪個作業員負責?
  `list_candidates(board='20085294') + query_board_context(board='20085294') + run_sql()`
- A32 工單對 open 的允收條件是怎麼寫的?
  `search_standards()`
- A33 哪一台機台 short 最多?查完幫我在系統裡開一張保養工單。
  `query_machine_stats(defect_type='short')` — matched: the grader's primary plan
...
listed-set rule, its two limits, no placeholders, 'this board' unnamed -- 2026-08-30
...
| should answer | 16 | 15/16 = 94% |
| should refuse | 9 | 9/9 = 100% |
| determinism | 25 | 22/25 = 88% planned the same tools across 3 runs |

**Held out from the prompt.** 5 of the 25 questions are few-shot examples verbatim or near-paraphrases, so on those the model is reciting rather than planning. On the remaining 20 it scored 20/20 = 100%, with 17/20 = 85% stable. Read that one rather than the headline above, and read it narrowly: it is agreement with one author's expected plans on question shapes that author chose. It does not bound the questions nobody thought to ask, the `days` and `top_k` arguments that go unscored, or whether the prose written over a correct plan is correct.

**Plans `validate_plan` threw out.** 2 of 25 did not validate, 2 of which had scored a hit on tools and arguments and so would be counted correct above while running nothing. The usual cause is a `days` beyond the 9 the store holds. Scoring counts the plan, so these are reported here rather than folded into the table.
...
- L3 的 spur 多不多，標準怎麼規定？
  `query_defect_history(defect_type='spur', line_id='L3') + search_standards()`
- 哪一台機器的缺陷率最高？
  `query_machine_stats(defect_type=None, top_n=1)` — matched: 2026-08-29's unclassed query_machine_stats ranks every machine by defects_per_board in one call; the two older alternatives predate it
- 去年同期的 open 缺陷率是多少？
  `(refused)`
- 2023 年第一季到現在的趨勢圖
  `(refused)`
...
| severity | questions | correct | should answer | should refuse |
|---|---|---|---|---|
| core | 51 | 33/51 = 65% | 18/28 = 64% | 15/23 = 65% |
| boundary | 18 | 12/18 = 67% | 10/13 = 77% | 2/5 = 40% |
| stretch | 1 | 0/1 = 0% | 0/1 = 0% | — |
...
**7 of the 70 cannot be passed by any plan at all, and are counted as misses above.** The grader pinned `defect_type` on a `search_standards`-only plan; `search_standards` takes `query` and has no such parameter, so `validate_plan` would throw out any plan that tried to satisfy the expectation. That is a grading error, recorded rather than repaired — the fixture marks them `fixture_defect` and a guard test asserts the list is exactly these. Excluding them, the score over the remaining 63 is 45/63 = 71%. Both numbers are here on purpose: the first is what the set as graded says, the second is what it says about the planner.
...
- S13 **stretch** 有一片 2008 開頭的，好像是 5294 還是 5249，客訴那批的，幫我找一下。 — matched no accepted plan: the grader's primary plan — never called ['query_board_context']; once the right stem is identified, listing its flagged regions is what '找一下' is for — never called ['list_candidates', 'query_board_context']
  planned: `run_sql()`
  graded: Both spellings are well-formed board ids and query_board_context returns a clean error for one that does not exist, so probing both resolves the question without guessing.
- S18 **boundary** 連續三片同一個位置有 open，我要不要停線？ — never queried defect_type=['open']
  planned: `run_sql() + search_standards()`
  graded: The documents carry the relevant rules — WI-201 makes any confirmed open critical, QP-110 escalates when a lot has produced two or more confirmed criticals, WI-300 escalates on repeat coordinates in the same lot — even though no document authorises a line stop.
  defect_type pinned on search_standards -- no tool in the expected plan takes that argument, so no plan can satisfy this expectation and the question scores a miss whatever the planner does
- S20 **core** 這片是誰判的？我要問他當初看到什麼。 — should have refused; planned run_sql()
  planned: `run_sql()`
...
- S08 去年同一個禮拜這條線的 defect rate 多少？
  `(refused)`
- S09 M22 怎樣
  `query_machine_stats(defect_type='open') + query_machine_stats(defect_type='short') + query_machine_stats(defect_type='mousebite') + query_machine_stats(defect_type='spur') + query_machine_stats(defect_type='copper') + query_machine_stats(defect_type='pin-hole') + query_defect_history(machine_id='M22') + query_defect_history(line_id='L2') + query_defect_history()` — matched: the grader's primary plan
- S10 那台新的上線以後 short 有沒有變多？
  `(refused)`
- S11 現在還有幾片卡在那邊等人看？
  `(refused)`
...
- S16 早班跟大夜的 false call 差很多嗎？同一台機器比。
  `(refused)`
- S17 copper 這個月變多，是真的變多還是我們判得比較嚴？
  `query_defect_history(date_from='2026-08-01', date_to='2026-08-09', defect_type='copper') + query_false_call_rate()` — matched: the grader's primary plan
- S18 連續三片同一個位置有 open，我要不要停線？
  `run_sql() + search_standards()`
- S19 L2 這個月的 defect rate 跟上個月比怎樣？
  `(refused)`
...
- A17 spur 跟 mousebite 長得很像,但我要問的是 spur,這幾天各機台的狀況?
  `query_machine_stats(defect_type='spur')`
- A18 M11 表現怎麼樣?
  `query_machine_stats(machine_id='M11') + query_defect_history(machine_id='M11')` — matched: the grader's primary plan
- A19 把這份 L2 的數據匯出成 Excel,寄給我主管。
  `(refused)`
- A20 品保問 mousebite 的允收標準,我們的工單有寫嗎?
  `search_standards()`
...
same prompt as the independent run above; the in-house fixture now accepts the unclassed one-call ranking -- 2026-08-30
...
**Held out from the prompt.** 6 of the 25 questions are few-shot examples verbatim or near-paraphrases, so on those the model is reciting rather than planning. On the remaining 19 it scored 18/19 = 95%, with 19/19 = 100% stable. Read that one rather than the headline above, and read it narrowly: it is agreement with one author's expected plans on question shapes that author chose. It does not bound the questions nobody thought to ask, the `days` and `top_k` arguments that go unscored, or whether the prose written over a correct plan is correct.
...
- 為什麼 M22 最近怪怪的？ — asked why, and never disclaimed cause
  planned: `query_defect_history(machine_id='M22') + query_machine_stats(defect_type='open')`
- 最差的那台機台，最近有沒有發生什麼事？ — refused a question it should have answered
  planned: `(refused)`
  graded: 2026-08-30: the machine is the top of a ranking over six listed machines, not a missing entity; events for all machines is one call
...
- L3 的 spur 多不多，標準怎麼規定？
  `query_defect_history(defect_type='spur', line_id='L3') + search_standards()`
- 哪一台機器的缺陷率最高？
  `query_machine_stats(defect_type=None, top_n=1)` — matched: 2026-08-29's unclassed query_machine_stats ranks every machine by defects_per_board in one call (defect_type absent -- a single classed call is one class, not every class)
- 去年同期的 open 缺陷率是多少？
  `(refused)`
- 2023 年第一季到現在的趨勢圖
  `(refused)`
...
- L1 的缺陷是不是變多了？
  `query_defect_history(line_id='L1') + query_defect_history(line_id='L2') + query_defect_history(line_id='L3')`
- M31 跟 M32 誰比較穩？
  `query_machine_stats(defect_type=None, top_n=2)`
- 2026-08-07 缺陷最多的前三台機器是哪幾台？
  `query_machine_stats(date_from='2026-08-07', date_to='2026-08-07', top_n=3)`
- 2026-07-30 缺陷數量前 5 名的機台
  `(refused)`
...
## 2026-08-30 · Adjudication — the listed-set rule, read against the three runs above

The planner gained one rule and one few-shot: an unnamed member of a small
enumerated set -- the worst machine, the line with the most opens, every
machine that had an event -- is planned as the ranking lookup plus the
dependent lookup for *every* member, not guessed and not refused. Three
in-house questions were added to hold it (two answerable, one that must
still refuse). The rule went through three wordings, each measured on both
fixtures before the next; only the last is in the prompt.

**Wording 1** (rule + few-shot): independent 26/42, 16/28, 62/70 stable.
Two over-applications. "哪一台機器的缺陷率最高" fanned out six history
calls where `query_machine_stats` ranks the set in one; and S20 "這片是誰判
的" -- which names no board -- wrote a SELECT 3/3, where the previous run
refused it. **Wording 2** added the two limits (a tool that ranks the set in
one call is the plan; boards, lots and reviewers are not listed sets):
28/42, 18/28, 59/70. The ranking question came back to one call; S20 did
not move, and the planner wrote `machine_id='<machine_id_from_previous_call>'`
into the dependent call twice. **Wording 3** (final) says outright that no
call's result can be passed to another and a placeholder is rejected before
anything runs, and puts "這片 without a stem names nothing" on the `run_sql`
rule itself:

| | 2026-08-29 (run 3, baseline) | final |
|---|---|---|
| independent, should answer | 28/42 | **28/42** |
| independent, should refuse (fixture's own key) | 15/28 | **17/28** |
| independent, same tools across 3 runs | 58/70 | **64/70** |
| independent, timeouts | 2 | 0 |
| in-house, should answer | -- | 14/16 |
| in-house, should refuse | -- | 9/9 |
| in-house, same tools across 3 runs | -- | 25/25 |

Flips against the baseline, adjudicated: S10, S11, A26 now refuse (unnamed
"that new machine", the waiting count, "the machine that had trouble") --
right; S17 and A18 now answered -- right, though A18's plan passes
`machine_id` to a tool without that parameter and `validate_plan` rejects
it, so on the page it is a refusal with the reason; S18 and S24 (a *position*
on the board, which no table holds) now refuse or write a SELECT that cannot
mean position -- the fixture called them answerable and the 08-29 adjudication
called S24 unanswerable, so the refusal is right and the SELECT is the S24
class of error the guard cannot see. **S20 is unmoved by any wording:** three
of three write a SELECT, and the final run's WHERE clause is
`b.id = '這片'` -- the two characters of "this board" as the board number.
`validate_plan` now refuses `<...>`, `{...}`, `YOUR_*` and `'*_ID'`
placeholders inside a SELECT as well as in typed arguments, and
`sql_guard` refuses `'這片'` at run time because no row holds it, so the page
shows a refusal either way; what the prompt could not buy is the refusal
*before* the SELECT was written.

The in-house miss that matters: "最差的那台機台，最近有沒有發生什麼事？"
refuses 3/3 under the final wording, after wording 1 answered it with the
events tool unfiltered and wording 2 answered it with a placeholder. The
planner wants to pass the ranking's top row into the second call, and the
architecture has no way to do that. That is the **dependent-argument**
mechanism -- a typed reference in the plan, resolved by the code after the
first branch returns and validated against the store's domains like any
other value -- and this question stays in the fixture as its standing
...
| | calls | median | mean | p90 | max |
|---|---|---|---|---|---|
| first 60s | 6 | 9.6s | 9.6s | 10.6s | 11.2s |
| steady state | 14 | 8.4s | 8.7s | 10.8s | 12.8s |
| all | 20 | 9.0s | 9.0s | 10.8s | 12.8s |

**Inside the deadline.** p90 is 10.8s against 60s, and 0 of 20 calls produced no explanation.

Against WI-300's 10s response budget, for reference and not as the verdict: 5 of 20 explanations took longer than the budget allows a *verdict* to take. No verdict waited on any of them — `classify_node` had already produced the disposition before the reason node was entered.

Of that service time, `eval_duration` accounts for 6.2s and prompt ingestion for 0.0s on average. The remaining 2.7s is thinking tokens, which Ollama generates and bills to nobody. Reporting `eval_ms` as the latency would have understated this run by 30%.
...
| | calls | median | mean | p90 | max |
|---|---|---|---|---|---|
| first 60s | 7 | 9.3s | 9.3s | 10.0s | 10.5s |
| steady state | 13 | 10.4s | 10.5s | 12.8s | 14.4s |
| all | 20 | 10.0s | 10.1s | 11.7s | 14.4s |

**Inside the deadline.** p90 is 11.7s against 60s, and 0 of 20 calls produced no explanation.

Against WI-300's 10s response budget, for reference and not as the verdict: 11 of 20 explanations took longer than the budget allows a *verdict* to take. No verdict waited on any of them — `classify_node` had already produced the disposition before the reason node was entered.

Of that service time, `eval_duration` accounts for 6.1s and prompt ingestion for 0.0s on average. The remaining 4.0s is thinking tokens, which Ollama generates and bills to nobody. Reporting `eval_ms` as the latency would have understated this run by 39%.

Queueing check: 0.0% of mean wall time is not load, prompt or generation — the request went straight to the GPU, so the run is not contended.

No request was served after an eviction.

## 2026-09-17 · commit 6c09a60

### Is the operator's rationale true of what the model was shown?

The verdict came off the LLM on 2026-08-23 and what it still writes -- the one sentence on a queue row an operator reads -- had never been scored. Two answers in this project's own question bank said so. 60 candidates the router sends to investigation, run in zh-TW and en, 24 min on `gpt-oss:20b`. Ground truth is not read: this scores the prose against the prompt, not against the board. `scripts/rationale_eval.py`.
...
| | | | |
| `class_not_named` | 0 | 0 | 0 |
| `other_class_named` | 4 | 6 | 10 |
| `limit_for_a_zero_tolerance_class` | 5 | 7 | 12 |

Clean by language: zh-TW 51/60, en 47/60.
...
2026-09-17 commit 6c09a60, --plan-only, local model comparison: gpt-oss:20b
...
| severity | questions | correct | should answer | should refuse |
|---|---|---|---|---|
| core | 51 | 31/51 = 61% | 16/28 = 57% | 15/23 = 65% |
| boundary | 18 | 9/18 = 50% | 7/13 = 54% | 2/5 = 40% |
| stretch | 1 | 0/1 = 0% | 0/1 = 0% | — |
...
**7 of the 70 cannot be passed by any plan at all, and are counted as misses above.** The grader pinned `defect_type` on a `search_standards`-only plan; `search_standards` takes `query` and has no such parameter, so `validate_plan` would throw out any plan that tried to satisfy the expectation. That is a grading error, recorded rather than repaired — the fixture marks them `fixture_defect` and a guard test asserts the list is exactly these. Excluding them, the score over the remaining 63 is 40/63 = 63%. Both numbers are here on purpose: the first is what the set as graded says, the second is what it says about the planner.
...
- S31 **core** Which station has the highest false call rate this week? — should have refused; planned query_false_call_rate()
  planned: `query_false_call_rate()`
  graded: query_machine_stats ranks machines by a real defect class only; false calls are excluded from it and from query_defect_history, so no false-call ranking exists.
- S32 **boundary** 我覺得最近人在看的量變多了，是不是機器越判越沒把握？ — should have refused; planned query_false_call_rate() + query_defect_history(machine_id=None)
  planned: `query_false_call_rate() + query_defect_history(machine_id=None)`
  graded: Both halves need history this surface does not have: escalation volume over time and the model's confidence distribution over time, neither exposed by any tool.
- S33 **core** short 要不要 100% 重驗？WI 有沒有寫？ — never queried defect_type=['short']
  planned: `search_standards()`
  graded: WI-202 speaks to this directly — no short is acceptable at any product class, and rework is followed by optical and electrical re-inspection — so the document either settles the argument or is shown not to state a 100% rule.
...
- S02 M21 跟 M22 這禮拜的 false call 差多少？
  `query_false_call_rate()`
- S03 20085294 這片客戶說有問題，當初我們是怎麼判的？
  `list_candidates(board='20085294') + query_board_context(board='20085294') + run_sql()` — matched: the grader's primary plan
- S04 看一下 L2
  `query_defect_history(line_id='L2')`
- S05 open 到什麼程度算 reject？WI 裡面怎麼寫的？
  `search_standards()`
...
- S08 去年同一個禮拜這條線的 defect rate 多少？
  `(refused)`
- S09 M22 怎樣
  `query_machine_stats(defect_type=None, top_n=None) + query_defect_history(machine_id='M22') + query_false_call_rate()` — matched: the grader's primary plan
- S10 那台新的上線以後 short 有沒有變多？
  `(refused)`
- S11 現在還有幾片卡在那邊等人看？
  `(refused)`
...
- S31 Which station has the highest false call rate this week?
  `query_false_call_rate()`
- S32 我覺得最近人在看的量變多了，是不是機器越判越沒把握？
  `query_false_call_rate() + query_defect_history(machine_id=None)`
- S33 short 要不要 100% 重驗？WI 有沒有寫？
  `search_standards()`
- S34 我們判 false call 放行的，後面有沒有被客戶抓到過？
  `(refused)`
- S35 我昨天休假，這兩天有沒有哪一台狀況要我特別注意的？
  `query_machine_stats(date_from='2026-08-07', date_to='2026-08-08', top_n=5)` — matched: the grader's primary plan
- A01 不要只看單一種缺陷,整體算下來這幾天哪一台機台最該排保養?
  `query_machine_stats(defect_type=None)`
- A02 上個月 L1 的 open 趨勢怎麼樣?有沒有愈來愈糟?
  `(refused)` — matched: reading '上個月' strictly as the calendar month before the data means zero overlap, which makes a refusal that states the 9-day limit defensible
...
- A17 spur 跟 mousebite 長得很像,但我要問的是 spur,這幾天各機台的狀況?
  `query_machine_stats(defect_type='spur')`
- A18 M11 表現怎麼樣?
  `query_defect_history(machine_id='M11') + query_machine_stats(defect_type=None)` — matched: the grader's primary plan
- A19 把這份 L2 的數據匯出成 Excel,寄給我主管。
  `(refused)`
- A20 品保問 mousebite 的允收標準,我們的工單有寫嗎?
  `search_standards()`
...
`nemotron-3-nano:30b-a3b-q4_K_M` at `think="low"`, 3 real reason-node calls over candidates the router sends to the LLM. The deadline is `EXPLANATION_DEADLINE_S`, 60s, and the run used it rather than overriding it — a call that misses it here is a call that produces no explanation in production. Explanations were written in `zh-TW` (`AOI_LINE_LANGUAGE`); a figure taken in one language says nothing about the other, since the same content is more tokens in Chinese than in English.
...
ollama ps before the run
NAME                              ID              SIZE     PROCESSOR    CONTEXT    UNTIL               
nemotron-3-nano:30b-a3b-q4_K_M    b725f1117407    24 GB    100% GPU     32768      29 minutes from now
...
|---|---|---|---|---|---|
| first 60s | 0 | — | — | — | — |
| steady state | 3 | 57.7s | 55.4s | 59.0s | 59.0s |
| all | 3 | 57.7s | 55.4s | 59.0s | 59.0s |

**Censored.** 17 of 20 calls hit the 60s deadline and wrote no explanation, so every figure in the table above is the distribution of the calls that survived. p90 of those is 59.0s. A deadline this model routinely misses is the signal WI-300 means: change the model, not the number.

Against WI-300's 10s response budget, for reference and not as the verdict: 3 of 3 explanations took longer than the budget allows a *verdict* to take. No verdict waited on any of them — `classify_node` had already produced the disposition before the reason node was entered.

Of that service time, `eval_duration` accounts for 8.0s and prompt ingestion for 3.2s on average. The remaining 44.2s is thinking tokens, which Ollama generates and bills to nobody. Reporting `eval_ms` as the latency would have understated this run by 80%.
...
The question the linear probe left open: is DINOv2 worse than the ResNet-18 on DeepPCB because of the backbone, or because a linear probe is the weakest way to use it? `dinov2_vits14` at `7764ea0f912e`, frozen (every backbone tensor hashed before and after each seed), 10 prompt tokens before each of its 12 blocks and a linear head on the class token -- 48,775 trainable parameters against the ResNet-18's 11.2 million. Trained by `train.fit` with the ResNet's split, augmentation, class weights, schedule and epoch rule; AdamW at 0.001 and batch 64, both fixed before the run. Stacked input, 64 -> 224 bilinear. Same 7,322 test candidates, 3,018 defects. Seeds 0, 1, 2, 147 min on `mps`. `scripts/dinov2_vpt.py`.

**Verdict, by the rule written before the run: Matches the ResNet-18: median oracle review removed 50.67% is inside its five seeds' 49.73%-52.79%.**

| seed | best epoch | oracle review removed | oracle escape | deployable review removed | deployable escape | backbone unchanged | minutes |
|---|---|---|---|---|---|---|---|
| 0 | 8 | **52.20%** | 0.50% | 30.88% | 0.13% | yes | 48 |
| 1 | 6 | **50.67%** | 0.50% | 32.85% | 0.13% | yes | 48 |
| 2 | 2 | **41.20%** | 0.50% | 19.84% | 0.07% | yes | 46 |
| **median** | — | **50.67%** | — | — | — | — | — |
| ResNet-18, five seeds (2026-08-31) | — | 49.73%–52.79% | — | — | — | — | — |
| DINOv2 linear probe, three seeds (2026-09-15) | — | 39.89%–42.04% | — | — | — | — | — |

The oracle column is each seed's best threshold at the ≤0.50% budget read on test: the column both comparators have at this cost, and not a deployment number. The deployable column is chosen on the seed's own validation split by the interval's upper bound and decides nothing. Three seeds here against the ResNet's five.

**What this does not establish.** Deep prompts only, at one prompt length, one learning rate and one batch size, none searched; shallow prompts, LoRA and full fine-tuning are untested. Stacked input only -- the probe's better configuration on this data was per-channel, which is three backward passes a candidate. The smallest backbone. And DeepPCB only.

## 2026-09-17 · commit 6c09a60

### Adjudication — which local model could replace `gpt-oss:20b`?

Read against the rule pre-registered in `77ac71a` ("Pre-registered: which local model could replace `gpt-oss:20b`?") and nothing added after it. One run, 11:53–16:46, arms grouped and never interleaved, everything unloaded between them; the per-script entries above are the evidence.

**Verdict: neither candidate can replace `gpt-oss:20b` on this machine, and both fail on time before quality is reached.**

| arm | a · gate 0 | b · deadline, zh-TW / en | c · rationale | d · planner | reading |
|---|---|---|---|---|---|
| `gpt-oss:20b` (control) | pass | **0 / 20** and **0 / 20** past 60 s; steady-state median 8.4 s / 10.4 s | clean zh-TW 51/60, en 47/60; `unsourced_figure` and `no_explanation` 0 | should answer 23/42, should refuse 17/28 | control |
| `qwen3:14b` | **fail** -- the real planning call passed the 60 s deadline (`ReadTimeout`); its explanation call took 31 s | not run | not run | not run | **not a replacement (a)** |
| `nemotron-3-nano:30b-a3b-q4_K_M` | pass | **17 / 20** past 60 s in zh-TW; **20 / 20** in en, which the latency script refused to publish | stopped at the 90-minute command limit, no entry | stopped at the 90-minute command limit, no entry | **not a replacement (b)** |

**What the failures are about.** `qwen3:14b` passed gate 0 on the same morning with a one-tool paraphrase of the planning prompt; on the real prompt it could not plan inside the deadline. That is the reason the gate uses the real prompt, and why the morning's pass is not evidence. `nemotron-3-nano` loaded and answered the schema, then spent the deadline: the three zh-TW calls that finished took a median 57.7 s. Swap on this 32 GB machine read 4.9 GB before its arm and over 15 GB after it, so what this measures is **this model on this machine** -- 24 GB of weights beside the rest of the workstation -- and not the model's speed on hardware that holds it. The pre-registered expectation about its Chinese was never reached: condition c did not run.

**What the control showed that was not asked.** `gpt-oss:20b` wrote its explanations faster than the 2026-08-30 entry (zh-TW steady-state median 8.4 s against 16.7 s then; en 10.4 s against 11.8 s), and its clean counts moved to zh-TW 51/60 and en 47/60 from 45 and 52. On the independent seventy it answered **23/42** against the 28/42 of 2026-08-30 with refusals unchanged at 17/28 -- a drop of five, outside the ±3 drift this project has measured on one model. The plans are in `models/analysis_eval_gpt-oss_20b.json`; the drop is named here and not explained, and it does not move the verdict, which compares candidates against this same-night control.

**What this does not establish.** One run. `synthesis_eval.py` and `agent_eval.py` were not run. Two candidates, one quantisation each, `think="low"` for both because it is the contract the station uses; a model that plans fast with thinking off is a different configuration and was not tried. `qwen2.5:14b` was excluded before the run for not taking `think`. And the answer to "why `gpt-oss:20b`" is now measured rather than asserted, but only in the sense the rule allows: of the models tried, it is the one that fits the deadline on this machine.
