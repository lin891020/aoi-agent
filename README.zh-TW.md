...
| **驗證** | 1,489 個測試，不需模型或 GPU。每個門檻皆引用出處；每個數字皆註明產生它的腳本。 |
...
大約 40 秒跑完 1,464 個不需要資料集的測試——不用 model、不用 GPU、不用資料集，除了
...
**1,489 個測試。** 其中 1,464 個在乾淨 checkout 上就能在 CI 跑完——它們自己在 tmpdir
