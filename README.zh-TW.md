...
| **結果** | **省去 55.6% 的人工複判**，門檻以 out-of-fold 選出，從未在報告它的那份切分上挑過。該門檻下 escape rate 為帶缺陷標籤 candidate 的 0.66%（95% 區間 0.43%–1.02%），對照 QP-110 的 0.5%：**在這個讀法下未達標**。若按缺陷計——QP-110 的原文寫法——複判模型漏判 0.35%，整線 0.51%。 |
| **技術** | Python 3.12 · PyTorch（MPS / CPU）· LangGraph · MCP · FastAPI + Jinja · SQLite · Ollama（`gpt-oss:20b`） |
| **驗證** | 1,382 個測試，不需模型或 GPU。每個門檻皆引用出處；每個數字皆註明產生它的腳本。 |
...
第二列是本文件在 2026-08-31 之前的頭條，而它正是缺陷所在：那個門檻是在同一份
切分上掃出來的，所以它的 ≤0.5% 達標是用答案買來的。下面的掃描表仍然保留，因為
作為**引擎之間**的比較，各給一個 oracle 是公平的——它不能成為的只是出貨數字。
...
- **選門檻的那份資料，預測不了出貨的那份。** 選出 0.912 的程序在 out-of-fold 上
  ...
  的——是 trainval 的板子比官方測試板容易，而這個模型上沒有任何門檻能補上這件事。
...
**1,382 個測試。** 其中 1,357 個在乾淨 checkout 上就能在 CI 跑完 —— 它們自己在
