...
- **有 9,140 筆判定早於這個歸屬欄位，而且它們自己說了。** 它們是 `unrecorded`，由
  ...
  放掉了多少。
- **這個專案的十五條不變式裡，有兩條只守住一半，還有一條根本守不住。**
  `CLAUDE.md` 列了十五條不能被悄悄改掉的規則；`scripts/invariant_audit.py` 會報出
  哪幾條真的會在被違反時讓測試失敗，而 `tests/test_invariant_audit.py` 會在某一條
  失去守衛時掛掉。十二條有守。fan-out 那條和官方 split 那條各自只守住一部分，而且逐條寫明守住的是哪一部分；「說清楚哪些是
